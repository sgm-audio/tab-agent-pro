...
class TestTabAgentPositions(unittest.TestCase):
    ...
    def test_open_string_position(self):
        """Open E2 (pitch 40) should return string 0, fret 0."""
        positions = self.agent.get_valid_positions(40)
        self.assertTrue(any(p["string"] == 0 and p["fret"] == 0 for p in positions))

    def test_open_string_position_multiple(self):
        """Pitch 45 appears on both A string (open) and low E (fret 5)."""
        positions = self.agent.get_valid_positions(45)
        self.assertTrue(any(p["string"] == 1 and p["fret"] == 0 for p in positions))
        self.assertTrue(any(p["string"] == 0 and p["fret"] == 5 for p in positions))
    ...
    def test_cost_encourages_same_string_fast(self):
        ...
        self.assertLess(cost, cost_other)
...
class TestTabAgentGenerateTab(unittest.TestCase):
    ...
    def test_empty_notes_returns_empty(self):
        ...
        self.assertEqual(len(self.agent.generate_tab([])), 0)

    def test_single_note_returns_position(self):
        """A single note produces one tab entry with a position."""
        notes = [make_note(64, 0.0)]  # E4
        result = self.agent.generate_tab(notes)
        self.assertEqual(len(result), 1)
        self.assertIn("string", result[0])
        self.assertIn("fret", result[0])
    ...
    def test_every_note_has_technique(self):
        ...
        for entry in result:
            self.assertIn("technique", entry)
            self.assertIn(entry["technique"], ["pick", "slide", "hammer", "pull"])

    def test_technique_detection_slide(self):
    ...
    def test_technique_detection_hammer_on(self):
        ...
        result = self.agent.generate_tab(notes, technique_sensitivity=0.9)
        techniques = [r["technique"] for r in result]
        self.assertIn("hammer", techniques)

    def test_technique_detection_pull_off(self):
    ...
    def test_drop_d_tuning(self):
        ...
        self.assertTrue(any(p["fret"] == 0 for p in positions))
